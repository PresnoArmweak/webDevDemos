    ...
    <div class="container">
        ...
        <?php if (isset($_GET['ok']) && $_GET['ok']==='1'): ?>
             <div class="alert alert-success">
                Thanks <?= esc_html($_GET['user']) ?>. Subscribed as <?= esc_html($_GET['email']) ?>
            </div>
        ...
        <?php if($_SERVER['REQUEST_METHOD'] !== 'POST' || $errors): ?>
            ...
            <form action="newsletter.php" method="post" class="mb-3">
                <label class="form-label">Username
                    <!-- sticky form -->
                    <input class="form-control" type="text" name="user" value=<?=esc_html((string)$user)?>>
                </label>
                <label class="form-label mt-2">Email
                    <input class="form-control" type="text" name="email" value=<?=esc_html((string)$email)?>>
                </label>
